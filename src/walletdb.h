...
public:
    ...
    bool WriteZDAZZSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZDAZZSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZDAZZSeed_deprecated(uint256& seed);
    bool EraseZDAZZSeed();
    bool EraseZDAZZSeed_deprecated();

    bool WriteZDAZZCount(const uint32_t& nCount);
    bool ReadZDAZZCount(uint32_t& nCount);
